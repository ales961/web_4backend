...
public class PointsEndpoint {
    ...
    public Response getAllPoints(@HeaderParam(HttpHeaders.AUTHORIZATION) String token) {
        try {
            return Response.ok(repository.findAllByUser(getUserFromJwt(token)))
                    .build();
        } catch (Exception e) {
        ...
        try {
            ...
            if (newPoint.getR() < 0)
                return Response.status(Response.Status.BAD_REQUEST)
                        .entity("R can not be negative!").build();
            PointEntity PointWithID = repository.create(newPoint);

            return Response.ok(new ObjWithMsgResponse<PointEntity>("PointEntity saved Successfully!", PointWithID))
                    .build();
        } catch (Exception e) {
            return Response.status(UNAUTHORIZED)
                    .build();
